...
function changePage(Dom) {
	Dom.Part1btn.find(".next").on("click", function () {
		...
		Dom.levNumLi.removeClass("selected");
		Dom.levNumLi.eq(1).addClass("selected");
		Dom.pageTitle.html("选择主机");
	});
	Dom.Part2btn.find(".pre").on("click", function () {
		$Animate.complete($Animate.Part2Hide1, $Animate.Part1Show);
		Dom.levNumLi.removeClass("selected");
		Dom.levNumLi.eq(0).addClass("selected");
		Dom.pageTitle.html("新建虚拟桌面");
	});
	Dom.Part2btn.find(".next").on("click", function () {
		$Animate.complete($Animate.Part2Hide2, $Animate.Part3Show);
		Dom.levNumLi.removeClass("selected");
		Dom.levNumLi.eq(2).addClass("selected");
		Dom.pageTitle.html("分配屏幕");
	});
	Dom.btnPart3.find(".pre").on("click", function () {
		$Animate.complete($Animate.Part3Hide, $Animate.Part2Show);
		Dom.levNumLi.removeClass("selected");
		Dom.levNumLi.eq(1).addClass("selected");
		Dom.pageTitle.html("分配屏幕");
	});
	Dom.btnPart3.find(".next").on("click", function () {
		$Animate.LayoutShow();
	});
}
